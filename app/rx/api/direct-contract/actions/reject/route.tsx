...
const rejectDirectContractSchema = z.object({
    contractId: z.string({
...
export async function POST(request: Request) {
    try {
        const reqBody = rejectDirectContractSchema.parse(await request.json());
        ...
        const acceptDirect = await prisma.directContract.update({
            ...
            data: {
                isEnded: true,
                endedBy: session?.user.email,
                endedReason: "Reject Invitation",
                endedAt: new Date(),
            },
